...
@cli.command()
@click.option('--robot-cls', default='cdspider_robot.robots.WxchatRobots', callback=load_cls, help='schedule name', show_default=True)
...
@click.pass_context
def wechat(ctx, robot_cls, aichat_rpc, uuid):
    """
    web wechat
    """
    aichat_rpc = connect_rpc(ctx, None, aichat_rpc)
    robot_cls = load_cls(ctx, None, robot_cls)
    robot = robot_cls(ctx, uuid=uuid)
...
@cli.command()
@click.option('--robot-cls', default='cdspider_robot.robots.AichatRobots', callback=load_cls, help='schedule name', show_default=True)
...
@click.pass_context
def aichat(ctx, robot_cls, uuid, bot_data, commands):
    """
    Aiml bot
    """
    robot_cls = load_cls(ctx, None, robot_cls)
    robot = robot_cls(ctx, commands = commands, bot_data = bot_data)
    robot.run(uuid)

@cli.command()
@click.option('--robot-cls', default='cdspider_robot.robots.AichatRobots', callback=load_cls, help='robot name', show_default=True)
@click.option('-b', '--bot-data', default=os.path.join(rdir, 'config', 'aiml'), help='AI头脑文件目录', show_default=True)
@click.option('-c', '--commands', multiple=True, help='commands')
...
@click.pass_context
def aichat_rpc(ctx, robot_cls, bot_data, commands, settings, xmlrpc_host, xmlrpc_port, debug):
    """
    Aiml bot rpc
    """
    robot_cls = load_cls(ctx, None, robot_cls)
    robot = robot_cls(ctx, commands = commands, bot_data = bot_data, settings = settings)
